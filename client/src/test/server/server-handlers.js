...
const handlers = [
  ...
  rest.post(`${authUrl}/register`, async (req, res, ctx) => {
    ...
    await usersDB.register(userFields);
    let user;
    try {
      user = await usersDB.login({ email, password });
    } catch (error) {
  ...
  rest.get(`${apiUrl}/books/:bookId`, async (req, res, ctx) => {
    ...
    if (!book) {
      return res(ctx.status(404), ctx.json({ status: 404, message: 'Book not found' }));
    }
...
function shouldFail(req) {
  ...
  const failureRate = Number(window.localStorage.getItem('__bookshelf_failure_rate__') || 0);
...
function requestMatchesFailConfig(req) {
  ...
  try {
    const failConfig = JSON.parse(window.localStorage.getItem('__bookshelf_request_fail_config__') || '[]');
...
//   const bookIdsInUsersList = (await listItemsDB.readByOwner(userId)).map((li) => li.bookId);
